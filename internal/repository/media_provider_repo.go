...
func NewMediaArchiveRepo(db *sql.DB) *MediaArchiveRepo {
	return &MediaArchiveRepo{db: db}
}

func (m *MediaArchiveRepo) GetDB() *sql.DB {
	return m.db
}

func (m *MediaArchiveRepo) SaveMediaArchive(tx *sql.Tx, objectID string, key string, filename string, extension string, sizeBytes int64, contentType string, probeData map[string]any) error {
...
func (m *MediaArchiveRepo) GetMediaArchiveByID(objectID string) (*entities.MediaArchiveRepo, error) {
	const query = `
		SELECT
			object_id,
			key,
	...
	var mediaArchive entities.MediaArchiveRepo
